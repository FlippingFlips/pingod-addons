...
public partial class PinGodGameProc : PinGodGame
{
    ...
    /// <summary>
    /// P-ROC version of the <see cref="MachineNode"/>
    /// </summary>
    ...
    private void SetupPinGodotFromProcGame()
    {
        ...
        if (PinGodProcGame != null)
        {
            ...
            if (PinGodProcGame.Switches?.Count > 0)
                PinGodProcGame.Switches.Values.ForEach(x => _procMachine.AddSwitch(x.Name, (byte)x.Number));
            if (PinGodProcGame.Coils?.Count > 0)
                PinGodProcGame.Coils.Values.ForEach(x => _procMachine.AddCoil(x.Name, (byte)x.Number));
            if (PinGodProcGame.Lamps?.Count > 0)
                PinGodProcGame.Lamps.Values.ForEach(x => _procMachine.AddLamp(x.Name, (byte)x.Number));
            if (PinGodProcGame.LEDS?.Count > 0)
                PinGodProcGame.LEDS.Values.ForEach(x => _procMachine.AddLed(x.Name, (byte)x.Number));
        }
    }
    ...
    private void StartProcGameLoop()
    {
        ...
        _procGameLoop = Task.Run(() =>
        {
            ...
            PinGodProcGame.RunLoop(1, tokenSource);
