...
namespace BasicGameGodot.addons.pingod_machine.tools
{
    public class EventRecordFile
    {
        ...
        public int GetQueueCount() => this._playbackQueue.Count;
        ...
        public PlaybackEvent ProcessQueue(ulong startTime)
        {
            ...
            if (nextEvt <= time)
            {
                ...
                Logger.Debug(nameof(PinGodMachine), ":playback evt ", pEvent.EvtName);
                return pEvent;
            }
        ...
        {
            var userDir = this.CreateRecordingsDirectory();
            _fileName = fileName;
            _recordFile = FileAccess.Open(fileName, FileAccess.ModeFlags.WriteRead);
            Logger.Info(nameof(PinGodGame), ":started recording to file: " + fileName);
        }
