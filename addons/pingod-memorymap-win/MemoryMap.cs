...
namespace PinGod.Core.Service
{
    ...
    public class MemoryMap : IDisposable
    {
        ...
        /// <summary> State for this and/or a simualtor </summary>
        ...
        private void CreateMutexAndMapping()
        {
            if (writeStates > -1 || readStates > -1)
            {
                ...
                //If you have run a simualtor like VP and it's still sitting in the editor then this mutex is still there...todo: remove it
                mutexCreated = System.Threading.Mutex.TryOpenExisting(MutexName, out mutex);
                if (!mutexCreated)
                {
                    Logger.Debug(nameof(MemoryMap), ":couldn't find existing mutex, creating a new one: ", MutexName);
                    mutex = new System.Threading.Mutex(true, MutexName, out mutexCreated);
                }
                else
                {
                    Logger.Debug(nameof(MemoryMap), ":found existing mutex:", MutexName);
                }
