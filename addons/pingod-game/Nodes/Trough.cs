...
public partial class Trough : Node
{
	...
	public override void _EnterTree()
	{
		...
		//switch commands
		if(pinGod !=null)
			pinGod.SwitchCommand += OnSwitchCommand;
	}
            ...
            if (pinGod != null)
			{
                ...
                if (pinGod.IsBallStarted && !pinGod.IsTilted && pinGod.BallSaveActive)
                {
                    for (int i = 0; i < TroughOptions?.EarlySaveSwitches?.Length; i++)
                    {
                        ...
                        {
                            FireEarlySave();
                            pinGod.EmitSignal(nameof(PinGodGame.BallSavedEventHandler));
                            break;
                        }
                    }
                }
			...
			if (pinGod.GameInPlay && !pinGod.BallStarted && !pinGod.IsTilted && !pinGod.IsMultiballRunning)
			{
				...
				if (_set_ball_save_on_plunger_lane)
				{
					...
					if (saveStarted)
					{
						...
						pinGod.EmitSignal(nameof(PinGodGame.BallSaveStartedEventHandler));
					}
				}
			}
		}
		...
		if (value > 0)
		{
			...
			if (troughFull && pinGod.IsBallStarted && !pinGod.IsMultiballRunning)
			{
				...
				else
				{
					...
					pinGod.EmitSignal(nameof(PinGodGame.BallDrainedEventHandler));
				}
			}
			//multiball
			else if (pinGod.IsMultiballRunning && !pinGod.BallSaveActive)
			{
				...
				if (TroughOptions.Switches.Length - 1 == balls)
				{
					...
					pinGod.EmitSignal(nameof(PinGodGame.MultiBallEndedEventHandler));
				}
			}
		}
		else
	...
	private void FireEarlySave()
	{
		...
		PulseTrough();
		pinGod.EmitSignal(nameof(BallSavedEventHandler));
	}
	...
	#region Timers
	/// <summary>
	/// Disables ball save and emits <see cref="PinGodBase.BallSaveEndedEventHandler"/>
	/// </summary>
	void _on_BallSaverTimer_timeout()
	{
		DisableBallSave();
		pinGod.EmitSignal(nameof(PinGodGame.BallSaveEndedEventHandler));
	}
